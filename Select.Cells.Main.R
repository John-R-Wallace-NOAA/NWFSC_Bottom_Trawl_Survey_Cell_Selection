        ...
        setwd("W:/ALL_USR/JRW/FRAM.Station.Pick/2024") # Change path as needed
        ...
        # NOT using the current year's seed and trying multiple times - since it's random,   change the value of the 'Density' factor for a First Alt. percent of 'positive distance above 2.25 (km)' of around 20-30%, and
        #          a Second Alt. 'percent of positive distance above 2.25 (km)' of around 55-75%.
        # Higher values of 'Density' give a higher percentage of alternative cells that are adjacent to the primary cell, and hence a lower percentage of alternative cells that are not adjacent to the primary cell (i.e. positive distances above 2.25 km).
        ...
        Den.list <- list(N.30.100 = 5.2, N.100.300 = 4.0, N.300.700 = 7.0, S.30.100 = 6.7, S.100.300 = 5.3, S.300.700 = 18.0)
               
        Verb <- c(TRUE, FALSE)[2]  #  Use verbose = TRUE to start, so that numbers can be checked. Then switch to FALSE to check multiple random runs.
        ...
        # Map primary and alts    (Primary.Alt.Cells used here before Lat/long removed below)     
        ...
        dev.new()
        histogram(~Distance.nm | factor(Order), data = Primary.Alt.Cells[Primary.Alt.Cells$Order %in% 2:3,])  # All distances for Order '1' are zero
        
        dev.new()
        par(mfrow = c(2,1))
        MASS::truehist(Primary.Alt.Cells$Distance.nm[Primary.Alt.Cells$Order %in% 2], xlim = c(0, 70), xlab = 'Distance Between Primary and 1st Alternative (nm)')
        MASS::truehist(Primary.Alt.Cells$Distance.nm[Primary.Alt.Cells$Order %in% 3], xlim = c(0, 70), xlab = 'Distance Between Primary and 2nd Alternative (nm)')

        
        # Output final results without cell corners 
        (Primary.Alt.Cells <- Primary.Alt.Cells[, c('Vessel', 'Tow.within.Vessel', 'Primary', 'Cent.ID', 'Distance.nm', 'Order')])[1:10, ] # Reorder columns and remove Lat/Long
        Primary.Alt.Cells[560:570, ]  # Check correctness of tow within vessel (3 * 188 = 564)
